...
export async function createCalendar(dto: CalendarDto): Promise<CalendarSchedule> {
  const res = await fetch(`${API_BASE}/calendar`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function updateCalendar(id: string, dto: CalendarDto): Promise<CalendarSchedule> {
